...
if (mes == 202105) {
 ...
 dataset[foto_mes==mes & Visa_fultimo_cierre== 5, Visa_fultimo_cierre :=  1 ]
 dataset[foto_mes==mes & Visa_fultimo_cierre > 5, Visa_fultimo_cierre := Visa_fultimo_cierre - 5 ]

 dataset[ foto_mes==mes & Master_fultimo_cierre== 5, Master_fultimo_cierre :=  1 ]
 dataset[ foto_mes==mes & Master_fultimo_cierre > 5, Master_fultimo_cierre := Master_fultimo_cierre -5]


#dataset[foto_mes==mes, Visa_Fvencimiento := Visa_Fvencimiento -61] 
#dataset[foto_mes==mes, Master_Fvencimiento := Master_Fvencimiento -61] 

#dataset[foto_mes==mes, Master_fechaalta := Master_fechaalta - 61] 
#dataset[foto_mes==mes, Visa_fechaalta :=  Visa_fechaalta - 61] 

#dataset[foto_mes==mes & cliente_antiguedad>=2, cliente_antiguedad := cliente_antiguedad -2]

#dataset[foto_mes==mes & Visa_Finiciomora >=61, Visa_Finiciomora := Visa_Finiciomora -61] 
#dataset[foto_mes==mes & Master_Finiciomora >=61 , Master_Finiciomora := Master_Finiciomora -61] 
...
#                    "Master_msaldototal", "Master_mconsumototal",
#                    "Visa_msaldototal","Visa_mconsumototal")

dataset <- dataset[,c("mcomisiones","mcuentas_saldo",
                    "Master_msaldototal", "Master_mconsumototal",
                    "Visa_msaldototal","Visa_mconsumototal"):=NULL]

#colnames(dataset)
#View(dataset[foto_mes==202103,])

#Guardar el archivo con las nuevas columnas
data.table::fwrite(dataset, file = "./datasets/data_norm_fechas.csv.gz")
#data.table::fwrite(dataset[,c("numero_de_cliente","foto_mes","Visa_fultimo_cierre","Master_fultimo_cierre",
#                               "Visa_Fvencimiento","Master_Fvencimiento","Master_fechaalta","Visa_fechaalta",
#                               "Visa_Finiciomora","Master_Finiciomora")], file = "./datasets/data_solo_fechas.csv")
