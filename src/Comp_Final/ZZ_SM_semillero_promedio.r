...
PARAM <- list()
PARAM$experimento <- "FINAL_01_ZZ_FT_15meses_RF_SinCan"
...
for (archivo in archivos) {
  ...
  if (vez == 1) {
    ...
    tb_ranking_semillerio <- tb_prediccion[, list( numero_de_cliente, rank, prob)]
    setnames(tb_ranking_semillerio, "rank", paste0("rank_", ksemilla))
    setnames(tb_ranking_semillerio, "prob", paste0("prob_", ksemilla))  
    vez <- 2
  } else {
