...
PARAM  <- list()
PARAM$experimento <- "FINAL_02_FEH_RF_3M_Can_50"

PARAM$exp_input  <- "FINAL_01_rank0_propia"
...
AgregaVarRandomForest  <- function( num.trees, max.depth, min.node.size, mtry)
{
  ...
  dataset_rf[ , entrenamiento := as.integer( foto_mes>= 202103 &  foto_mes<= 202105 & ( clase01==1 | azar < 0.10 )) ]
...
{
  ncol( dataset )
  CanaritosAsesinos( canaritos_ratio = 0.5)
  ncol( dataset )
}
