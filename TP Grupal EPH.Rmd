...
**NIVEL_ED:**   Nivel educativo del individuo. <br>
      1 = PRIMARIO INCOMPLETO  <BR>
      2 = PRIMARIO COMPLETO    <BR>
      3 = SECUNDARIO INCOMPLETO  <BR>
      4 = SECUNDARIO COMPLETO   <BR>
      5 = UNIVERSIT INCOMPLETO  <BR>
      6 = UNIVERSIT COMPLETO    <BR>
...
```
Armamos un dataset con los individuos en rol HIJO, mayores de edad (>=18 años), en estado OCUPADO y con ingreso por actividad principal >0
```{r}
# HIJOS
...
View(base)  
```
Ahora hacemos algunas transformaciones para limpiar el dataset fina. <br>
```{r}
...
View(base)
```
Eliminamos aquellos registros que tengan valor nulo en el ingreso de JEFE/A y MADRE/PADRE (P21_1 y en P21_6)  <br>
```{r}
base_final <- base %>%
...
Aplicamos esta convención para evitar las difenentes modalidades de escolaridad polimodal, egb, etc. <br>
Convención: <br>
...
base_final <- base_final %>% 
  mutate(anios_estudio = case_when(NIVEL_ED == 1 ~ 4,
                                   ...
                                   NIVEL_ED == 9 ~0))
```
En el análisis premilinar de los datos se observaron 4 registros con valores de ingreso principal muy alejado de la media. <br>
Borro los hogares outliers <br>
```{r}
base_final <- base_final %>%
...
```
Verificación de supuesto de correlación entre la variable instrumento y la covariable. <br>
Aplicamos el índice  de correlación de Spearman. <br>
```{r warning=FALSE}
library(corrr)
...
```
Se verifica que la correlación es baja pero no es cero. <br>
El otro supuesto que debe cumplir es el de exogeneidad: que la variable instrumento no tenga efecto directo sobre la variable objetivo. <br>
...
<br>
Vamos a verificar la distribución de la variable target: Ingreso actividad principal (P21)
<br>
```{r}
...
```
<br>
Generalmente se usa una transformación logarítmica para modificar la forma de la distribución. <br>
Se observa una mayor normalidad usando esta transformación por lo que de acá en adelante utilizaremos el modelo log-nivel. <br>
...
metricas = metrics(data = modelo_a, truth = P21, estimate = fitted_antilog) %>% mutate(.estimate = round(.estimate, 3))
metricas

```
Sin sorpresas, da un R cuadrado bajísimo (2.5%), al no cumplirse algunos supuestos del modelo lineal (que comentaremos a continuación) y el problema de endogeneidad a tratar con variable instrumental.
<br>
...
En ambos modelos los $\beta_1$ estimados son muy significativos (p-valor << 0.05). <br>
En el Modelo con la Variable Instrumental del Ingreso de los Padres, el efecto marginal de los años de educación sobre el salario es 10 veces mayor al del Modelo sin tratar la endogeneidad. En el [Wooldridge, 2010] también la magnitud del $\beta_1$ de MC2E es superior al del MCO.
